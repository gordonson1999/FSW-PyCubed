...
class Resource:
    ...
    def acquire(self, chip_select):
        assert self.active_cs is None, (
            "cannot acquire owned resource.  Owned cs: "
            + self.active_cs
            + ", requested cs: "
            + chip_select
        )
    ...
    def release(self, chip_select):
        assert self.active_cs is not None, (
            "cannot release unowned resource.  Owned cs: "
            + self.active_cs
            + ", requested cs: "
            + chip_select
        )
...
class TestManagedResource(TestCase):
    def test_acquire(self):
        ...
        self.assertEqual(
            len(loop._tasks), 1
        )  # 2 is suspended, not eligible to be run next step

        loop._step()  # 1 after context
        self.assertIsNone(spi.active_cs)
        self.assertEqual(
            len(loop._tasks), 2
        )  # 1 is unfinished, 2 is unsuspended by the ManagedResource

        loop._step()  # 1 end           2 work
        self.assertIs(spi.active_cs, 2)
        self.assertEqual(
            len(loop._tasks), 1
        )  # 1 is finished, 2 is working with spi on cs2
